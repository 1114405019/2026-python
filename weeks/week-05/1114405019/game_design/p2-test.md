...
| 測試名稱 | 輸入 | 預期輸出 |
|---------|------|---------|
| test_empty_cards | `[]` | `None` |
| test_classify_invalid_combo | `[♠A,♥K,♣Q,♦J,♠T,♥9]` (6張) | `None` |
| test_cant_form_pair | `[♠A,♥K]` | `None` |
| test_five_cards_none | `[♠A,♥K,♣Q,♦J,♠T]` (無法組成任何五張牌型) | `None` |
...
- **檢查方式**:
  ...
  coverage run -m unittest tests.test_classifier
  coverage report
  ```

### 邊界情況重點
1. 牌數邊界：0 張、1 張、2 張、3 張、5 張、6+ 張
2. 順序邊界：最小順子 (A-2-3-4-5) vs 最大順子 (T-J-Q-K-A)
3. 相同牌型比較：數字相同時比花色
4. 不同牌型比較：直接按優先級比較