...
from bigtwo.game.models import Card, Deck, Player, CardType
from bigtwo.game.classifier import HandClassifier
from bigtwo.game.finder import HandFinder
from bigtwo.game.ai import AIStrategy
from bigtwo.game.models import CardType
...
class BigTwoGame:
    ...
    def ai_turn(self) -> bool:
        ...
        # 出牌
        success = self.play(best_play)
        
        if not success:
            # 如果出牌失敗（例如判定邏輯有 Bug），強制過牌以防遊戲卡死
            print(f"DEBUG: AI {current_player.name} 出牌失敗，強制過牌。")
            self.pass_turn()
